...
//    var fy = 400;
    ...
    document.getElementById('hhh').value = hhh.toFixed(2);
...
var downloadAsFile = function(fileName, content) {
    ...
    textArray[10]   = document.getElementById('hhh').value ;
...
function OpenFile(){
	...
	reader.onload = function(ev){
	    ...
	    document.getElementById('hhh').value = textArray[10]   ;
...
function OnButtonClick(){
    ...
    var hhh = Number(document.getElementById('hhh').value);
    ...
    var yg;
    if( idBound == 0 ) {
	mtop = hh/(2*beta)*(1.0+beta*hhh);
	mmax = -hh*hhh*Math.pow(1.0+beta*hhh,2)/(2.0*beta*hhh);
	mmax = mmax * Math.exp(-Math.atan(1.0/(beta*hhh)));
	//
	Lm = 1.0/beta*Math.atan(1.0/(beta*hhh));
	//
	y0 = hh*Math.pow(hhh,3)/(12.0*e*1000*iner);
	y0 = y0 * ( Math.pow(1.0+beta*hhh,3)+2.0 ) / (Math.pow(beta*hhh,3)) ;
	//
	yg = hh*Math.pow(hhh,3)/(4.0*e*1000*iner);
	yg = yg * (1.0+beta*hhh)/Math.pow(beta*hhh,3);
	//
	L0 = 1.0/beta*Math.atan( (beta*hhh+1.0)/(beta*hhh-1.0) );

    } else {
	mtop = 0.0;
	//
	mmax = -hh*hhh* Math.sqrt( Math.pow( 1.0+2.0*beta*hhh , 2 ) + 1.0 ) / (2.0*beta*hhh) ;
	mmax = mmax * Math.exp( Math.atan( 1.0/(1.0+2.00*beta*hhh) ) );
	//
	Lm = 1/beta*Math.atan( 1.0/(1.0+2.0*beta*hhh) );
	//
	y0 = hh*Math.pow(hhh,3)/(3.0*e*1000*iner);
	y0 = y0*( Math.pow(1+beta*hhh,3) + 0.5 ) / Math.pow( beta*hhh, 3);
	//
	yg = hh*Math.pow(hhh,3)/(2.0*e*1000*iner);
	yg = yg* (1.0+beta*hhh)/Math.pow(beta*hhh,3);
	//
	L0 = 1/beta* Math.atan( (1+beta*hhh)/(beta*hhh) );
    }
    ...
    result += 'Lateral deflection at the ground surface,';
    result += '</td><td>';
    result += 'y<sub>G,L</sub> = ';
    result += (yg*100).toFixed(6);
    result += 'cm';
    ...
    var fig = '';
    if( idBound == 0 ) {
	fig += '<p><img src="images/Fix2.jpg" alt="Location" width="220px" /><br  /></p>';
    }
    if( idBound == 1 ) {
	fig += '<p><img src="images/Pin2.jpg" alt="Location" width="220px" /><br  /></p>';
    }
